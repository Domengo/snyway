    ...
    if (RdReg(pAce->FaxClassType, FCLASS_MAJ) == FCLASS2)
    {
        switch (pAce->CurrCommand[pAce->ATidx++])
        {
            case '=':
                ...
                {
                    pAce->ATidx++;
                    AceSetResponse(pTable, ACE_FRY_CAP);
                    return SUCCESS;
                }
                else
                {
                    ...
                    if (COMM_AsciiToUByte((UBYTE *)pAce->CurrCommand, &pAce->ATidx, &fry, &isEnd, TRUE, 0))
                    {
                        pAce->pDteToT30->ecmRetryCnt = fry;
                        return SUCCESS;
                    }
                }
            ...
            case '?':
            {
                ...
                COMM_HexToAscii(DteRd, pAce->pDteToT30->ecmRetryCnt);
