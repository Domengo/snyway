    ...
    if (RdReg(pAce->FaxClassType, FCLASS_MAJ) == FCLASS2)
    {
        switch (pAce->CurrCommand[pAce->ATidx++])
        {
            case '=':
                ...
                switch (pAce->CurrCommand[pAce->ATidx++])
                {
                    case '0': pAce->pDteToT30->isDocForPolling = FALSE; TRACE0("ACE: No document to poll"); return SUCCESS;
                    case '1': pAce->pDteToT30->isDocForPolling = TRUE; TRACE0("ACE: Document ready for polling"); return SUCCESS;
            ...
            case '?':
            {
                ...
                PutByteToCB(DteRd, pAce->pDteToT30->isDocForPolling ? '1' : '0');
